...
auth0 = oauth.register(
    ...
    access_token_url="https://" + os.environ.get(
        'AUTH_DOMAIN') + "/oauth/token",
...
def search_wine_with_mutiple():
        ...
        if len(params_dict) == 1:
            for key, value in params_dict.items():
                if key != "cases_produced":
                    ...
                    sql_str += key + ">= {}".format(
                        start) + " and " + key + "<= {}".format(end)
        else:
            for key in params_list:
                _index = params_list.index(key) + 1
                if _index != len(params_list):
                    if key != "cases_produced":
                        sql_str += key + " like '%{}%' and ".format(
                            params_dict[key])
                    else:
                        start = cases_produced.split("-")[0]
                        end = cases_produced.split("-")[1]
                        sql_str += key + ">= {}".format(
                            start) + " and " + key + "<= {} and".format(end)
                else:
                    if key != "cases_produced":
                        sql_str += key + " like '%{}%' ".format(
                            params_dict[key])
                    else:
                        start = cases_produced.split("-")[0]
                        end = cases_produced.split("-")[1]
    ...
    res_list = []
    for i in res:
        res_dict = {"bottle_id": i[0], "winery_name": i[1], "winery_id": i[2],
                    "year": i[3], "wine_name": i[4],
                    "pct_alcohol": i[5], "ta": i[6], "ph": i[7], "soils": i[8],
                    "varietals": i[9], "clones": i[10],
                    "clusters": i[11], "aging_process": i[12],
                    "cases_produced": i[13], "source_file": i[14],
...
def techsheets_by_id(bottle_id):
    ...
    cursor.execute(
        "SELECT a.*,b.winery_url FROM bottle_data AS a RIGHT JOIN winery_data AS b ON a.winery_name = b.winery_name WHERE a.bottle_id = {};".format(
            bottle_id))
    query_results = cursor.fetchall()
...
def varietal_names_in_bottle_data():
    ...
    query_results = cursor.fetchall()
    for query_result in query_results:
        varietals = re.split(',|and', query_result[0]) + re.split(',|and',
                                                                  query_result[
                                                                      1])
        for varietal in varietals:
            ...
            if varietal not in returnable and varietal != "NULL" and varietal != "Null" and len(
                    varietal) > 0:
...
def add_or_cancel_fav_wineries():
    ...
    cursor.execute(
        "select * from favorite_winery where user_id ={} and fav_winery = {}".format(
            user_id, winery_id))
    res = cursor.fetchall()
    print(res)
    # cancle
    if len(res) != 0:
        cursor.execute(
            "delete from favorite_winery where user_id = {} and fav_winery={}".format(
                user_id, winery_id))
        db.commit()
        response = {
            "code": 200,
    ...
    # add
        ...
        cursor.execute(
            "insert into favorite_winery (user_id,fav_winery) values ({},{})".format(
                user_id, winery_id))
        db.commit()
        response = {
            "code": 200,
...
def list_fav_wineries():
    ...
    cursor.execute(
        "select * from favorite_winery where user_id = {}".format(user_id))
    res = cursor.fetchall()
    arr = []
    for i in res:
        winery_id = i[1]
        cursor.execute(
            "select winery_name from winery_data where winery_id = {}".format(
                winery_id))
...
def add_or_cancel_fav_qualities():
    ...
    cursor.execute(
        "select * from favorite_quality where user_id = {} and fav_wine = {} and fav_quality_str = '{}'".format(
    ...
    if len(res) != 0:
        ...
        cursor.execute(
            "insert into favorite_quality (user_id,fav_wine,fav_quality_str) values({},{},'{}')".format(
                user_id, wine_id, qualitStr))
        db.commit()
        response = {
            "code": 200,
...
def list_fav_qualities():
    ...
    user_id = request.args.get("userId")
    cursor.execute(
        "select * from favorite_quality where user_id = {}".format(user_id))
    res = cursor.fetchall()
    response = {}
    for i in res:
        wine_id = i[1]
        quality_str = i[2]
        cursor.execute("select {} from bottle_data where bottle_id ={}".format(
            quality_str, wine_id))
...
def searchWineWithQuality():
    ...
    if request.args.get("produced") != None:
        ...
        sql_str += "and cases_produced >= {} and cases_produced <={}".format(
            start_va, end_va)
