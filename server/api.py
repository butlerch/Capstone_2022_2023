...
def search_wine_with_mutiple():
        ...
        if len(params_dict) == 1:
            for key, value in params_dict.items():
                if key == "pct_alcohol":
                    ...
                    # sql_str += "CAST(REPLACE(pct_alcohol, '%', '') AS NUMERIC) BETWEEN {} AND {}".format(start, end)
                    query_list.append(
                        "CAST(REPLACE(pct_alcohol, '%', '') AS NUMERIC) BETWEEN {} AND {}".format(start, end))
                elif key != "cases_produced":
                    # sql_str += key + " like '%{}%'".format(value)
                    query_list.append(key + " like '%{}%'".format(value))

                else:
                    if "_" in cases_produced:
                        start = cases_produced.split("-")[0]
                        end = cases_produced.split("-")[1]
                    elif "+" in cases_produced:
                        start = cases_produced.split("+")[0]
                        end = 100000
                    else:
                        start = 0
                    ...
                    query_list.append(key + ">= {}".format(start) + " and " + key + "<= {}".format(end))
        else:
            for key in params_list:
                ...
                if _index != len(params_list):
                    ...
                    if key == "pct_alcohol":
                        ...
                        query_list.append(key + " like '%{}%'".format(params_dict[key]))
                    else:
                        if "_" in cases_produced:
                            start = cases_produced.split("-")[0]
                            end = cases_produced.split("-")[1]
                    ...
                    if key == "pct_alcohol":
                        ...
                        # sql_str += "CAST(REPLACE(pct_alcohol, '%', '') AS NUMERIC) BETWEEN {} AND {}".format(start,
                        #                                                                                       end)
                        query_list.append(
                            "CAST(REPLACE(pct_alcohol, '%', '') AS NUMERIC) BETWEEN {} AND {}".format(start, end))
                    elif key != "cases_produced":
    ...
    if len(query_list) != 0:
        query_sql += " where " + " and ".join(query_list)
