...
def make_a_returnable_single(query_results, colnames):
    '''
...
def add_wine():
    ...
    if allowed_file(pdf.filename) and os.path.exists(filename) is False:
        pdf.save(os.path.join(pathway, filename))
    ...
    cleaned = make_a_returnable_single(data, colnames)
