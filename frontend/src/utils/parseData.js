...
export function parseOverview(techsheetData) {
    ...
    if (techsheetData && techsheetData["wine_name"] && techsheetData["wine_name"].length > 0 && techsheetData["wine_name"][0] !== "NULL") {
        overview.subtitle = cleanString(techsheetData["wine_name"][0]);
        for (let i = 1; i < techsheetData["wine_name"].length; i++) {
            overview.subtitle = overview.subtitle + ", " + cleanString(techsheetData["wine_name"][i]);
        }
    }
...
export function parseFilePaths(techsheetData, fileDirectory = "/pdfs/", fileFormat = ".pdf", thumbnailDirectory = "/thumbnails/", thumbnailFormat = ".png") {
    /* Error handling.*/
    if (!techsheetData["source_file"] || techsheetData["source_file"].length < 6) {
        return false;
    }
...
export function parseNeighbors(techsheetData, path = "/winesheets/") {
    ...
    if (techsheetData["bottle_id"] > 1 && techsheetData["bottle_id"] < totalTechsheets) {
...
export function parseTechnicalData(techsheetData) {
    ...
    if (techsheetData && techsheetData["aging_process"] && techsheetData["wine_name"].length > 0 && techsheetData["aging_process"] !== "NULL") {
        technicalData.push({
            "property": "Aging Process",
            "value": techsheetData["aging_process"][0].toUpperCase() + techsheetData["aging_process"].substring(1)
        })
    }

    /* Parse cases_produced, an integer.*/
    if (Number.isInteger(techsheetData["cases_produced"])) {
        technicalData.push({"property": "Cases Produced", "value": techsheetData["cases_produced"]})
    }

    /* Parse clones, a string array.*/
    /* Create a string based on the array contents.*/
    let clonesString = "";
    if (techsheetData["clones"].length > 0 && techsheetData["clones"][0] !== "NULL") {
        ...
        if (techsheetData["clones"].length > 1) {
            for (let i = 1; i < techsheetData["clones"].length; i++) {
                clonesString = clonesString + ", " + cleanString(techsheetData["clones"][i]);
            }
        }
    }
    ...
    if (clonesString.length > 0) {
        technicalData.push({"property": "Clones", "value": clonesString})
    }

    /* Parse clusters, a string.*/
    if (techsheetData["clusters"] !== "NULL" && techsheetData["clusters"] !== undefined) {
        technicalData.push({"property": "Clusters", "value": cleanString(techsheetData["clusters"])})
    }

    /* Parse % Alcohol, a string.*/
    if (techsheetData["pct_alcohol"] !== "NULL" && techsheetData["pct_alcohol"] !== undefined) {
        technicalData.push({"property": "Alcohol Percentage", "value": cleanString(techsheetData["pct_alcohol"])})
    }
    ...
    let cleanSoils = (techsheetData["soils"].filter((element) => element.trim() !== "")).map((element) => cleanString(element))
    ...
    if (soilsString.length > 0) {
        technicalData.push({"property": "Soils", "value": soilsString})
    }

    /* Parse TA, a string.*/
    if (techsheetData["ta"] !== "NULL" && techsheetData["ta"] !== undefined) {
        technicalData.push({"property": "TA", "value": cleanString(techsheetData["ta"])})
    }

    /* Parse varietals, a string array.*/
    /* Create a string based on the array contents.*/
    let varietalsString = "";
    if (techsheetData["varietals"].length > 0 && techsheetData["varietals"][0] !== "NULL") {
        ...
        if (techsheetData["varietals"].length > 1) {
            for (let i = 1; i < techsheetData["varietals"].length; i++) {
                varietalsString = varietalsString + ", " + cleanString(techsheetData["varietals"][i])
            }
        }
    }
    ...
    if (varietalsString.length > 0) {
        technicalData.push({"property": "Varietals", "value": varietalsString})
    }
...
export function parseCategory(list) {
    ...
    let cleanList = (list.filter((element) => element.trim() !== "")).map((element) => cleanString(element))
    if (cleanList.length > 0 && cleanList[0] !== "NULL" && cleanList[0] !== undefined) {
        string = cleanList[0]
...
function encodeFileName(string="Crème Brulée"){
    ...
    return encodeURIComponent(string.normalize("NFD").replace(/[\u0300-\u036f]/g, ""));
